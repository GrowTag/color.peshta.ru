		...
		else
		{
		    if($arProperty["NAME"] == "Размер" or
                ...
                foreach ($arProperty["VALUES"] as $valueID => $arValue)
                {
                    ...
                    ?>
                    <input <?=$checked?> onchange='return CRamCalc.OnListItemClick(this);' type='radio' name='<?=$arProperty["FIELD_NAME"]?>' value='<?=$valueID?>'/>&nbsp;<?=$arValue["TITLE"]?>
                    <?
                }
		    } else {?>
